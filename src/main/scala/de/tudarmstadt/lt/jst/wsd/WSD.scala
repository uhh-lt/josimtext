...
import de.tudarmstadt.lt.jst.utils.{Const, KryoDiskSerializer, Util}
...
object WSD {
    ...
    def main(args: Array[String]) {
        if (args.size < 4) {
            println("Usage: clusters coocs deps trigrams lexsample output [de.tudarmstadt.lt.jst.wsd-mode] [use-prior-prob] [max-num-features] [partitions-num]")
            return
        }
