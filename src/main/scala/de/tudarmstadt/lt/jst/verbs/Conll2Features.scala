package de.tudarmstadt.lt.jst.verbs

import de.tudarmstadt.lt.jst.corpus.ReformatConll
...
object Conll2Features {
  ...
  val verbose = false
  val conllRecordDelimiter = ReformatConll.newConllRecordDelimiter
  ...
  def main(args: Array[String]) {
    if (args.size < 3) {
      println("Parameters: <input-dir> <output-dir> <de.tudarmstadt.lt.jst.verbs-only>")
      println("<input-dir>\tDirectory with a parsed de.tudarmstadt.lt.jst.corpus in the CoNLL format.'")
      println("<output-dir>\tDirectory with an output word feature files")
      println("<de.tudarmstadt.lt.jst.verbs-only>\tIf true features for de.tudarmstadt.lt.jst.verbs are saved only.")
      return
    }
