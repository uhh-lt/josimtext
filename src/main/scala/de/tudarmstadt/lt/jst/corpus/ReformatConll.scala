...
object ReformatConll {
    ...
    def main(args: Array[String]) {
      if (args.size < 2) {
        ...
        println(s"<input-dir>\tDirectory with a parsed de.tudarmstadt.lt.jst.corpus in the CoNLL format: delimiter='$oldConllRecordDelimiter'.")
        println(s"<output-dir>\tDirectory with a parsed de.tudarmstadt.lt.jst.corpus in the CoNLL format: delimiter='$newConllRecordDelimiter'.")
        return
      }
